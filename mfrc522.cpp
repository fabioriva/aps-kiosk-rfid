...
//-------------------------------------------------------------------------------------------------
static byte SPI_transfer(byte value)
{
	byte result, mask;
	result = 0;
...
{
	//SPI_beginTransaction(SPISettings(SPI_CLOCK_DIV4, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
	SPI_beginTransaction();							// Set the settings to work with SPI bus
	//digitalWrite(_chipSelectPin, LOW);		// Select slave
	SPI_transfer(reg & 0x7E);				// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	SPI_transfer(value);
	//digitalWrite(_chipSelectPin, HIGH);		// Release slave again
	SPI_endTransaction(); // Stop using the SPI bus
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_WriteRegisters(byte reg,		///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisters(byte reg,		///< The register to read from. One of the PCD_Register enums.
	byte count,		///< The number of bytes to read
	byte* values,	///< Byte array to store the values in.
	byte rxAlign	///< Only bit positions rxAlign..7 in values[0] are updated.
)
{
	byte address, index, mask, i, value;
	if (count == 0) {
		return;
	}
	...
	while (index < count)
	{
		...
		{		// Only update bit positions rxAlign..7 in values[0]
			...
			mask = 0;
			for (i = rxAlign; i <= 7; i++) {
				mask = uint8_t(mask | (1 << i));
			}
			// Read value and tell that we want to read the same address again.
			value = SPI_transfer(address);
			// Apply mask to both current value of values[0] and the new data in value.
			values[0] = uint8_t((values[index] & ~mask) | (value & mask));
		}
		else { // Normal case
...
void PCD_ClearRegisterBitMask(byte reg,	///< The register to update. One of the PCD_Register enums.
	...
	byte tmp;
	tmp = PCD_ReadRegister(reg);
	PCD_WriteRegister(reg, uint8_t(tmp & (~mask)));		// clear bit mask
...
/*	pinMode(_chipSelectPin, OUTPUT);
	...
	/*	strcpy(LcdString,(char *)"  Wait Lettore  ");
		LcdLineaScr(1,1);
		strcpy(LcdString,(char *)"  Transponder   ");
		LcdLineaScr(2,1);*/
	resetok = FALSE;
	i = 0;
...
void PCD_ReInit(void)
{
	...
	PCD_WriteRegister(ModeReg, 0x3D);		// Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO 14443-3 part 6.2.4)
	PCD_AntennaOn();
	///PCD_SetAntennaGain(0x70);        SU ALCUNI LETTORI DA FASTIDIO
}
...
byte PCD_Reset(void)
{
	...
	while (PCD_ReadRegister(CommandReg) & (1 << 4))
	{
		...
		nretray++;
		if (nretray > 100)
			return FALSE;		// dopo timeout 1sec. esce comunque
	}
	delay(1);
	return TRUE;
} // End PCD_Reset()
...
)
{
	...
	byte bitFraming = uint8_t((rxAlign << 4) + txLastBits);		// RxAlign = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]
	...
	// se lettura nuova carta poche letture altrimenti troppo lento, altre operazioni lungo altrimenti non fa in tempo
	if (FlagNewCard == 1)
		i = 20;
	else	i = 2000;
	...
	if (backData && backLen && checkCRC)
	{
		...
		status = PCD_CalculateCRC(&backData[0], uint8_t(*backLen - 2), &controlBuffer[0]);
		if (status != STATUS_OK) {
			return status;
		}
...
byte PICC_Select(Uid* uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	while (!uidComplete) {
		...
		currentLevelKnownBits = (signed char)(validBits - (8 * uidIndex));
		...
		if (useCascadeTag) {
			currentLevelKnownBits = (signed char)(currentLevelKnownBits + 8);
		}
		...
		while (!selectDone) {
			...
			if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level are known. This is a SELECT.
				...
				index = uint8_t(2 + count);					// Number of whole bytes: SEL + NVB + UIDs
			...
			if (result == STATUS_COLLISION) { // More than one PICC in the field => collision.
				...
				count = uint8_t((currentLevelKnownBits - 1) % 8); // The bit to modify
				index = uint8_t(1 + (currentLevelKnownBits / 8) + (count ? 1 : 0)); // First byte is index 0.
				buffer[index] = uint8_t(buffer[index] | (1 << count));
			}
 ...
 */
													 ...
													 )
 {
	 ...
	 cmdBuffer[1] = page;
	 //memcpy((void*)&cmdBuffer[2], (void*)buffer, 4);
		 for(i=0;i<4;i++)
	   ...
	   * @return STATUS_OK on success, STATUS_??? otherwise.
		...
		*/
		 ...
		 */
		  ...
		  * @return STATUS_OK on success, STATUS_??? otherwise.
		  */
		  /*byte PCD_NTAG216_AUTH(byte* passWord, byte pACK[]) //Authenticate with 32bit password
		  {
			  byte result, i;
...
)
{
	...
	//memcpy((void*)cmdBuffer,(void*)sendData, sendLen);
	for (i = 0; i < sendLen; i++)
  ...
  */
   ...
   */
	...
	*/
	...
	//	response[32]; // Card's response is written here
		received;
		status = PCD_TransceiveData(&cmd, (byte)1, response, &received, &validBits, (byte)0, FALSE); // 40
		if(status != STATUS_OK)
		{
			//if(logErrors) {
	...
	*/
	 ...
	 */
	 /*byte MIFARE_SetUid(byte *newUid, byte uidSize, byte logErrors)
	 {
		 ...
		 status = PCD_Authenticate(PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key, &(mfrc522.uid));
		 if (status != STATUS_OK) {
	 ...
	 //			  PICC_WakeupA(atqa_answer, &atqa_size);
		 ...
		 // Read block 0
	 //	byte block0_buffer[18];
		 byteCount = sizeof(block0_buffer);
...
{
	byte result;
	result = PICC_Select(&(mfrc522.uid), 0);
	return (result == STATUS_OK);
} // End
//-------------------------------------------------------------------------------------------------
/*
	static struct timespec TimeCicloMs;
	pinMode(LED1, OUTPUT);
	pinMode(LED2, OUTPUT);
	uint8_t value;
	SetTimeout(&TimeCicloMs, 10);
	while(1)
	{
		if(CheckTimeoutExpired(TimeCicloMs) == 1)
		{
			// tempo ciclo 3000 ms
			SetTimeout(&TimeCicloMs, 3000);
			value = !value;
			if(value)
